...
describe('parseInstagramZip', () => {
    ...
    it('throws MissingFilesError when followers_*.json is absent', async () => {
        await expect(parseInstagramZip(fixture('missing-followers.zip'))).rejects.toThrow(MissingFilesError);
    ...
    it('throws MissingFilesError when following.json is absent', async () => {
        await expect(parseInstagramZip(fixture('missing-following.zip'))).rejects.toThrow(MissingFilesError);
    });
    ...
    it('throws SchemaValidationError when followers JSON has wrong structure', async () => {
        await expect(parseInstagramZip(fixture('invalid-schema.zip'))).rejects.toThrow(SchemaValidationError);
    ...
    it('sets followedAt to null when timestamp is 0', async () => {
        ...
        z.file('connections/followers_and_following/following.json', JSON.stringify({ relationships_following: [] }));
