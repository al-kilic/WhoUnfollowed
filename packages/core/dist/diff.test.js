...
function makeSnapshot(followers, following, exportedAt = 1700000000) {
    return {
        exportedAt,
...
describe('analyzeSnapshot', () => {
    ...
    it('returns all results sorted alphabetically', () => {
        ...
        const result = analyzeSnapshot(snapshot);
        const isSorted = (arr) => arr.every((v, i) => i === 0 || arr[i - 1].username <= v.username);
...
describe('findGhostFollowers', () => {
    ...
    function makeSnapshotWithTimestamps(followers, following) {
        return {
    ...
    it('excludes followers with null followedAt (HTML exports)', () => {
        const snapshot = makeSnapshotWithTimestamps([{ username: 'unknown_date', followedAt: null }], []);
        const ghosts = findGhostFollowers(snapshot);
        expect(ghosts).toHaveLength(0);
    });
    it('respects custom minTenureDays option', () => {
        const fiftyDaysAgo = Math.floor(Date.now() / 1000) - 50 * 86400;
        const snapshot = makeSnapshotWithTimestamps([{ username: 'borderline', followedAt: fiftyDaysAgo }], []);
    ...
    it('returns empty array when there are no ghost followers', () => {
        const snapshot = makeSnapshotWithTimestamps([{ username: 'mutual', followedAt: OLD_TIMESTAMP }], ['mutual']);
