...
const entries: { version: string; date: string; tag: 'launch' | 'fix' | 'improvement' | 'feature'; items: string[] }[] = [
  {
    ...
    tag: 'improvement',
    items: [
      'Light and dark mode. Toggle in the top nav on every page.',
      'Works properly on mobile now. Every page, every section.',
      'New feature tour on the Results and Radar pages. Shows up once per session and walks you through what each section does.',
      'Tap any date range on Pending Requests to sort those accounts to the top of the list.',
      'Each pending request now shows a status based on how long it has actually been waiting, not a generic label for all of them.',
      'The Follow Ratio card on Radar now explains what the number means and how to improve it.',
      'Hovering the filter pills on your results list now tells you what each label actually does.',
      'New What is WhoUnfollowed page with a side-by-side comparison against every other app in this space.',
      'New About and Contact pages.',
      'Privacy Policy updated with full legal structure and explicit platform independence statement.',
    ],
  },
  {
    version: '0.5',
