...
import type { ResolvedComparison } from '@/app/compare/comparisons';
import { Icon } from '@/components/landing/atoms';
import type { CompareDetailUiContent } from './detailContent';

export function CompareDetailContent({ c, others, ui }: {
  c: ResolvedComparison;
  others: ResolvedComparison[];
  ui: CompareDetailUiContent;
}) {
  return (
    <div style={{ minHeight: '100vh', background: T.bg, color: T.ink, fontFamily: T.sans }}>
      ...
      <main className="px-4 sm:px-8" style={{ maxWidth: 720, margin: '0 auto', paddingTop: 56, paddingBottom: 80 }}>
        ...
        <div style={{ marginBottom: 40 }}>
          ...
          <p style={{ fontSize: 11, fontFamily: T.mono, color: T.tealMid, letterSpacing: '0.16em', textTransform: 'uppercase', margin: '0 0 8px' }}>{ui.whoUnfollowedVs}</p>
        ...
        <div style={{ borderRadius: 16, border: `1px solid ${T.border1}`, overflow: 'hidden', marginBottom: 40 }}>
          <div className="grid grid-cols-3" style={{ background: T.surface1, borderBottom: `1px solid ${T.border1}` }}>
            <div style={{ padding: '12px 16px', fontSize: 11, color: T.inkMute, fontFamily: T.mono, letterSpacing: '0.08em' }}>{ui.featureHeader}</div>
        ...
        <div style={{ padding: '20px 22px', borderRadius: 14, background: T.surface1, border: `1px solid ${T.border2}`, marginBottom: 32 }}>
          <div style={{ fontSize: 10, color: T.tealMid, fontFamily: T.mono, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 10 }}>{ui.verdictLabel}</div>
        ...
        <div style={{ marginBottom: 40, padding: '18px 20px', borderRadius: 14, background: T.surface1, border: `1px solid ${T.border1}` }}>
          <div style={{ fontSize: 10, color: T.tealMid, fontFamily: T.mono, letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 8 }}>{ui.relatedReadingLabel}</div>
        ...
        {others.length > 0 && (
          <div>
            <div style={{ fontSize: 10, color: T.inkMute, fontFamily: T.mono, letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: 12 }}>{ui.moreComparisonsLabel}</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {others.map(o => (
                <Link key={o.slug} href={`/compare/${o.slug}`} style={{ textDecoration: 'none', padding: '14px 18px', borderRadius: 12, background: T.surface1, border: `1px solid ${T.border1}`, display: 'block' }}
                  ...
                  <div style={{ fontSize: 10, color: T.inkMute, fontFamily: T.mono, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>{ui.vsPrefix}</div>
